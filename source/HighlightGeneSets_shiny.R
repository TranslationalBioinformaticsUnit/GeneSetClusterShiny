...
HighlightGeneSets_shiny <- function(Object, highligt.genes, name = "Ros")
{
  message("[=========================================================]")
  ...
  for(cl.i in unique(Object@Data[[1]]$cluster))
  {
    ...
    for(genes.cl.i in 1:nrow(DF.cl))
    {
      ...
      if(unique(as.character(Object@metadata[,"structure"])) == "SYMBOL"){
        ...
        genes.df <- bitr(genes.x, fromType = unique(as.character(Object@metadata[,"structure"])), toType = c("SYMBOL"), OrgDb = unique(as.character(Object@metadata[,"organism"])))
