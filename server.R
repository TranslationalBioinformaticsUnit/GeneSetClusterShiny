...
server <- function(input, output, session) {
  ...
  observeEvent(input$printResults, {
    ...
    if(((is.null(input$source))||(is.null(input$files))||(is.null(input$gene.id))||(is.null(input$organism)))&&(!runexample())){
      ...
      if ((input$source=="Great") || (input$source=="GSEA") || (is.null(input$source))){
        ...
        if((is.null(input$source))||(runexample())){
          #run example
          filesexample<-c("./example/MM10.GREAT.KO.uGvsMac.bed.tsv", "./example/MM10.GREAT.WT.uGvsMac.bed.tsv")
          #message("filess server11111")
          #message(filesexample)
          #message("------------")
          load.object <- LoadGeneSets_shiny(file_location = filesexample,
                                            groupnames= c("KO","WT"),
                                            P.cutoff = 0.05,
                                            Mol.cutoff = 5,
                                            Source = "Great",
      ...
      if(tocontinue){
        ...
        if (length(unique(manage.object@metadata$seperator))!=1){
          ...
          cluster.object <- ClusterGeneSets(Object = combine.object,
                                            ...
                                            method = "Hierarchical", order="cluster")
          ...
          if(dim(cluster.object@Data[[1]]["Pathways"])[1]>200){
            output$treeIntro<-renderText("Treeplot not performed because size greater than 200.")
          }else{
            treeplotoutput<<-reactive({PlotTree(Object = cluster.object, clusters = optimalCluster, doORA = F, nodenames = F, wordcloud = wordcloudperform)})
            output$treeplot<-renderPlot(treeplotoutput())
          }
          #plot heatmap
          heatmapoutput<<-reactive({PlotGeneSets(cluster.object, doORA = F, wordcloud = wordcloudperform)})
          ...
          for (i in (1:max(as.numeric(cluster.object@plot$aka2$Cluster)))){
            ...
            if (i==1){
              ...
              colnames(genesFinal)[i+1]<- paste("Cluster_",i, " (Terms=", totalPathways,")", sep="")
            }else{
  ...
  observeEvent(input$recalculateClustering, {
    ...
    if(dim(data.object()@Data[[1]]["Pathways"])[1]>200){
      output$treeIntro<-renderText("Treeplot not performed because size greater than 200.")
    }else{
      treeplotoutput<<-reactive({PlotTree(Object = data.object(), clusters = input$cluster, doORA = F, nodenames = F, wordcloud = wordcloudperform)})
      output$treeplot<-renderPlot(treeplotoutput())
    }

    #plot new heatmap
    #heatmapoutput<<-reactive({PlotGeneSets(Object = data.object(), fontsize = 5, main= "", legend=TRUE, annotation.mol=FALSE, RR.max=50)})
    heatmapoutput<<-reactive({PlotGeneSets(Object = data.object(), doORA = F, wordcloud = wordcloudperform)})
    output$heatmap<-renderPlot(heatmapoutput())
