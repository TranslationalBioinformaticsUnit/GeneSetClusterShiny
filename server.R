...
#setwd("/Users/ruizdeam/Desktop/Github/Ewoud_GeneSetCluster/shiny/v15")
...
server <- function(input, output, session) {
  ...
  observeEvent(input$recalculateClustering, {
    ...
    if(dim(data.object()@Data[[1]]["Pathways"])[1]>200){
      ...
      treeplotoutput<<-reactive({PlotTree(Object = data.object(), clusters = optimalCluster, doORA = F, nodenames = F, wordcloud = T)})
      output$treeplot<-renderPlot(treeplotoutput())
    }
