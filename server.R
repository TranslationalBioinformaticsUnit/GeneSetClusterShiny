...
source("source/PlotTreeShiny_2.R")
    ...
    if(((is.null(input$source))||(is.null(input$files))||(is.null(input$gene.id))||(is.null(input$organism)))&&(!runexample())){
      ...
      if(tocontinue){
        ...
        if (length(unique(manage.object@metadata$seperator))!=1){
          ...
          #plot heatmap
          heatmapoutput<<-reactive({PlotGeneSets(Object = cluster.object, fontsize = 5, main= "", legend=TRUE, annotation.mol=FALSE, RR.max=50)})
  ...
  output$download.plots<-downloadHandler(
    ...
    content = function(file) {
      ...
      if((is.null(treeplotoutput())) & (is.null(tissueoutput()))){
        ...
        ggsave(plot = heatmapoutput(), filename = filenames[1], width = 30, unit = "cm", device = input$format)
